...
/* structure about IP header */
struct ip_hdr {
    uint8_t vhl;      /* version and header length */
    uint8_t tos;      /* Type Of Service */
    uint16_t total;   /* Total Length (about datagram) */
    uint16_t id;      /* identification */
    uint16_t offset;  /* flag nad flagment offset */
    uint8_t ttl;      /* Time To Live */
    uint8_t protocol; /* Protocol number (ex.IP=4) */
...
struct ip_protocol {
    ...
    uint8_t type; /* type of protocols (ex. TCP, UDP) */
...
struct ip_route {
    ...
    ip_addr_t netmask; /* subset mask */
    ip_addr_t nexthop;
    struct ip_iface *iface; /* interface */
};
 ...
 * protext these lists with a mutex. */
static struct ip_iface *ifaces;
static struct ip_protocol *protocols;
static struct ip_route *routes; /* routing table */

/* Change IP address from strings to binary(ip_addr_t) which is Network Byte
 * Order. */
...
/* Change IP address from binary(ip_addr_t) which is Network Byte Order to
 * strings. */
...
/* NOTE: must not be call after net_run() */
int ip_route_set_default_gataway(struct ip_iface *iface, const char *gateway) {
    ip_addr_t gw;
...
int ip_iface_register(struct net_device *dev, struct ip_iface *iface) {
    ...
    iface->next = ifaces;
    ifaces = iface;
    infof("registered: dev=%s, unicast=%s, netmask=%s, broadcacst=%s",
          dev->name, ip_addr_ntop(iface->unicast, addr1, sizeof(addr1)),
          ip_addr_ntop(iface->netmask, addr2, sizeof(addr2)),
          ip_addr_ntop(iface->broadcast, addr3, sizeof(addr3)));
    return 0;
}

/* Serch for IP interface */
...
int ip_protocol_register(uint8_t type,
    ...
    for (entry = protocols; entry; entry = entry->next) {
        if (entry->type == type) {
            errorf("already exsists, exist=%u, type=%u", entry->type, type);
            return -1;
        }
    }
...
static void ip_input(const uint8_t *data, size_t len, struct net_device *dev) {
    ...
    /* Filtering IP datagram*/
    iface = (struct ip_iface *)net_device_get_iface(dev, NET_IFACE_FAMILY_IP);
    if (!iface) {
        errorf("iface(interface) is not resister to the device");
        return;
    }
...
static ssize_t ip_output_core(struct ip_iface *iface, uint8_t protocol,
    ...
    hdr->src = src;
    hdr->dst = dst;
    hdr->sum = cksum16((uint16_t *)hdr, hlen, 0); /* don't convert byte oder*/
