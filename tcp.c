...
struct tcp_hdr {
    ...
    uint8_t off;  /* Data offset (4bit) */
    uint8_t flg;  /* Flags (6bit) */
    uint16_t wnd; /* Window */
...
struct tcp_pcb {
    ...
    struct {
        uint32_t nxt; /* Sequence number to send next */
        uint32_t una; /* Last sequence number which doesn't reply ACK */
        uint16_t wnd; /* recv window (destination) */
        uint16_t up;  /* emergency pointer */
        uint32_t wl1; /* sequence number */
        uint32_t wl2; /* ACK number */
    } snd;
    uint32_t iss;
    struct {
        uint32_t nxt;
    ...
    uint32_t irs;       /* Partner's initial sequence number */
    uint16_t mtu;       /* MTU by sender device */
    uint16_t mss;       /* MAK Segment Size */
...
static void tcp_segment_arrives(struct tcp_segment_info *seg, uint8_t flags,
    ...
    switch (pcb->state) {
        case TCP_PCB_STATE_LISTEN:
            ...
            if (TCP_FLG_ISSET(flags, TCP_FLG_SYN)) {
                 ...
                 * state, but processing of SYN and ACK should not be repeated
                 */
                return;
            }
            /*
             * 4th check for text or control
             */
        ...
        case TCP_PCB_STATE_SYN_SENT:
            ...
            if (TCP_FLG_ISSET(flags, TCP_FLG_SYN)) {
                ...
                if (pcb->snd.una > pcb->iss) {
                    ...
                    pcb->state = TCP_PCB_STATE_SYN_RECEIVED;
                    tcp_output(pcb, TCP_FLG_SYN | TCP_FLG_ACK, NULL, 0);
                    /* ignore: if there are other contrils or text in the
    ...
    switch (pcb->state) {
        ...
        case TCP_PCB_STATE_LAST_ACK:
             ...
             * SYN and FIN), and RCV.NXT.  Segments with higher begining
             * sequence numbers may be held for later processing.
             */
    }
    /*
     * 2nd check the RST bit
    ...
    switch (pcb->state) {
        ...
        case TCP_PCB_STATE_CLOSE_WAIT:
            if (pcb->snd.una < seg->ack && seg->ack <= pcb->snd.nxt) {
                pcb->snd.una = seg->ack;
                tcp_retransmit_queue_cleanup(pcb);
                /* ignore: Users should receive positibe acknowledgements for
...
static void tcp_input(const uint8_t *data, size_t len, ip_addr_t src,
    ...
    if (cksum16((uint16_t *)hdr, len, psum) != 0) {
        errorf("checksum error: sum=0x%04x, cerify=0x%04x", ntoh16(hdr->sum),
