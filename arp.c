...
struct arp_ether_ip {
    struct arp_hdr hdr;
    uint8_t sha[ETHER_ADDR_LEN]; /* Sendr Hardware Address */
...
/* ARP Cache Structure */
struct arp_cache {
    unsigned char state;        /* Cache Status */
    ip_addr_t pa;               /* Protocol Address */
    uint8_t ha[ETHER_ADDR_LEN]; /* Hardware Address */
    struct timeval timestamp;   /* Latest Update Time */
};
...
static struct arp_cache *arp_cache_alloc(void) {
    struct arp_cache *entry, *odlset = NULL;

    for (entry = caches; entry < tailof(caches); entry++) {
        if (entry->state == ARP_CACHE_STATE_FREE) {
            return entry;
        }
        if (!odlset || timercmp(&odlset->timestamp, &entry->timestamp, >)) {
            odlset = entry;
        }
    }
    arp_cache_delete(odlset);
    return odlset;
}

/* Serch ARP cache (pa means IP address)*/
...
static int arp_reply(struct net_iface *iface, const uint8_t *tha, ip_addr_t tpa,
    ...
    debugf("dev=%s, opcode=%s(0x%04x), len=%zu", iface->dev->name,
           arp_opcode_ntoa(reply.hdr.op), ntoh16(reply.hdr.op), sizeof(reply));
