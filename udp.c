...
struct udp_pcb {
    int state;
    struct ip_endpoint local; /* own IP address and Port number */
    struct queue_head queue;  /* recieve queue */
    struct sched_ctx *ctx;
};

struct udp_queue_entry {
    struct ip_endpoint foreign; /* Target IP address and Port number */
    uint16_t len;
    uint8_t data[];
};
...
int udp_init(void) {
    ...
    if (net_event_subscribe(event_handler, NULL) == -1) {
        errorf("net_event_handler() failure");
        return -1;
    }
    return 0;
}
...
ssize_t udp_recvfrom(int id, uint8_t *buf, size_t size,
    ...
    memcpy(buf, entry + 1, len); /* entry->data doesn't work */
    memory_free(entry);
    return len;
}