...
static int ether_tap_addr(struct net_device *dev) {
    ...
    mempcpy(dev->addr, ifr.ifr_hwaddr.sa_data, ETHER_ADDR_LEN);
    close(soc);
    return 0;
}
...
static int ether_tap_open(struct net_device *dev) {
    ...
    /* Set Asynchoronous I/O signal delivery destination */
...
static size_t ether_tap_write(struct net_device *dev, const uint8_t *frame,
                              size_t flen) {
...
struct net_device *ether_tap_init(const char *name, const char *addr) {
    ...
    if (addr) {
        if (ether_addr_pton(addr, dev->addr) == -1) {
            errorf("incaild address, addr=%s", addr);
            return NULL;
        }
    }
